...
def build_report(db, payload, user_id):
    period_start = to_utc_naive(payload.period_start)
    period_end = to_utc_naive(payload.period_end)
...
def render_html(report):
    d = as_dict(report)
    return f"""<!doctype html><html><head><meta charset='utf-8'><style>@font-face{{font-family:'SecureOps CJK';src:url('file:///usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc');}}body{{font-family:'SecureOps CJK','Noto Sans CJK SC',sans-serif;color:#172033;padding:40px}}h1{{color:#1261a0}}.grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}}.card{{padding:18px;background:#eef7ff;border-radius:8px}}table{{width:100%;border-collapse:collapse;margin-top:20px}}th,td{{padding:10px;border-bottom:1px solid #ddd;text-align:left}}</style></head><body><h1>{escape(report.report_name)}</h1><p>生成时间：{format_shanghai(report.generated_at)}</p><p>统计周期：{format_shanghai(report.period_start)} 至 {format_shanghai(report.period_end)}</p><div class='grid'><div class='card'>资产总数<br><b>{report.asset_count}</b></div><div class='card'>漏洞总数<br><b>{report.vulnerability_count}</b></div><div class='card'>CVE 数量<br><b>{report.cve_count}</b></div><div class='card'>高危漏洞<br><b>{report.high_risk_vulnerability_count}</b></div></div><h2>风险等级分布</h2><pre>{json.dumps(d['risk_distribution'],ensure_ascii=False,indent=2)}</pre><h2>扫描情况</h2><pre>{json.dumps(d['scan_statistics'],ensure_ascii=False,indent=2)}</pre><h2>整改建议</h2><ul>{''.join(f'<li>{escape(x)}</li>' for x in d['recommendations'])}</ul></body></html>"""
